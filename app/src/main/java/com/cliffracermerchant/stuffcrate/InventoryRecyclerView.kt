...
class InventoryRecyclerView(context: Context, attributes: AttributeSet) :
    ...
    fun setViewModel(owner: LifecycleOwner, viewModel: InventoryViewModel) {
        ...
        /*val selectionState = viewModel.getSavedSelection()
        val observer = object: Observer<List<Long>> {
            override fun onChanged(t: List<Long>?) {
                if (t != null) selection.restoreState(t)
                Log.d("savestate", "state restored")
                viewModel.clearSavedSelection()
                selectionState.removeObserver(this)
            }
        }
        selectionState.observeForever(observer)*/
    ...
    fun deleteItemAtPos(pos: Int) = deleteItems(pos)//listDiffer.currentList[pos].id)

    fun deleteItems(vararg positions: Int) {//vararg ids: Long) {
        val expandedPos = expandedItemAdapterPos
        if (expandedPos != null && expandedPos in positions)//listDiffer.currentList[expandedPos].id in ids)
