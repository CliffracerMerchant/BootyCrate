/* Below is a copy of the BottomAppBarTopEdgeTreatment.java file from the
 | Android Open Source Project with the following modifications:
 | - The constructor no longer checks if the FAB vertical offset is less than
 |   0, and consequently will no longer throw an IllegalArgumentException if
 |   this is the case.
 | - The getEdgePath function checks if the FAB vertical offset is less than 0.
 |   If this is the case, an alternative shape path is drawn to support FABs
 |   with a less than 0 vertical offset. */
...
public class BottomAppBarTopEdgeTreatment extends EdgeTreatment implements Cloneable {
  ...
  public void getEdgePath(
    ...
    if (verticalOffset < 0) {
      shapePath.lineTo(middle - roundedCornerOffset, 0);
      shapePath.addArc(/* left= */ middle - cradleRadius - 2 * roundedCornerOffset,
                       /* top= */ 0,
                       /* right= */ middle - cradleRadius,
                       /* bottom= */ 2 * roundedCornerOffset,
                       /* startAngle= */ ANGLE_UP,
                       /* sweepAngle= */ ARC_QUARTER);
      shapePath.lineTo(middle - cradleRadius, roundedCornerOffset - verticalOffset);
      shapePath.addArc(/* left= */ middle - cradleRadius,
                       /* top= */ -cradleRadius - verticalOffset,
                       /* right= */ middle + cradleRadius,
                       /* bottom= */ cradleRadius - verticalOffset,
                       /* startAngle= */ ANGLE_LEFT,
                       /* sweepAngle= */ -ARC_HALF);
      shapePath.lineTo(middle + cradleRadius, roundedCornerOffset);
      shapePath.addArc(/* left= */ middle + cradleRadius,
                       /* top= */ 0,
                       /* right= */ middle + cradleRadius + 2 * roundedCornerOffset,
